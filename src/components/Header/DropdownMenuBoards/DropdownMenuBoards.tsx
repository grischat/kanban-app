...
export default function DropdownMenuBoards() {
    ...
    const allBoards = useSelector(
        (state: RootStateCurrentBoard) => state.boardsColumnsReducer.allBoards
    );
    const theme = useSelector(
        (state: RootStateTheme) => state.switchThemeReducer.theme
    );
    const dispatch = useDispatch();
    const handleSelectedItem = (board: Board<string>) => {
        dispatch({
            type: 'selectBoard',
    ...
    return (
        <Dropdown open={open}>
            ...
            <Menu className={`boardsMenu-${theme}`}>
                ...
                {allBoards.map((el: Board<string>, index) => (
                    <MenuItem
                        onClick={() => {
